...
- **模型费用类表格里的价格标签改成「提示」而不是「状态」**：概览的费用占比与分析页的
  模型明细只在需要你动手时才挂标签 —— 没价的（`未定价`，费用会算成 0）和估算的
  （`估算价`，按同族价猜的、要你核对）。官方价 / `手填` / models.dev 目录价都是能直接
  用的价，一律不挂，否则一列里全是徽章，真正该处理的那行反而看不见了。
  请求详情抽屉的「单价规则」与单价表照旧显示来源 —— 那里"这条请求按哪个价算的"
  本身就是内容，不是提示。
